    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
...

            {
                ...
                List<string> str = new List<string> { "12345", "123", "123456", "1" };
            ...
            {    
                ...
                //TODO+
                List<string> str = new List<string> { "1qwerty", "cqwertyc", "cqwe", "c" };
                string ans = str.Aggregate((x, y) => x.First().ToString()  + y.First().ToString());
                ans = str.Select(x => x.First().ToString()).Aggregate((x, y) => x + y);
            }
...

            {
                ...
                int k = 3;
                IEnumerable<string> n = new string[] { "11", "241B", "234S", "431", "H12", "ABC", "123" };
                var res = n.Where(x => x.Length == k && char.IsDigit(x.Last())).OrderBy(x => x);
            }

            {
                ...
                int K = 3;
                int D = 296;
                IEnumerable<int> A = new int[] {7, 84, 75, 38, 296, 7, 93, 4, 8, 90, 347, 6, 90, 23, 84 };

                var ans = A.TakeWhile(x => x < D).Concat(A.Skip(K)).Distinct().OrderByDescending(x => x);  
            }
...

            {
                ...
                //TODO+
                IEnumerable<string> n = new string[] { "Fgh1", "sggf2", "Ftwth3", "Fthv4", "G5", "GthHkjh6", "noplkj7" };

                var res = n.Select(x => x.Length % 2 != 0 ? x.First() : x.Last()).OrderByDescending(x => x);
            }

            {
                ...
                int K1 = 100;
                int K2 = 50;
                IEnumerable<int> A = new int[] { 7, 834, 75, 38, 296, 7, 93, 4, 8, 90, 347, 6, 90, 23, 84 };
                IEnumerable<int> B = new int[] { 1, 223, 55, 1, 41, 48, 922, 264, 1853, 9537, 285, 3, 950, 423, 684 };

                var ans = A.Where(x => x > K1).Concat(B.Where(x => x < K2)).OrderBy(x => x);
            }
            {
            ...
            }
            {
                ...
                //TODO+
                IEnumerable<string> A = new string[] { "11", "241B", "234S", "431", "H12", "ABC", "123" };
                IEnumerable<string> B = new string[] { "1", "241", "234", "43", "H1", "AB", "12" };

                var res = A.OrderBy(x => x).Join(B.OrderByDescending(x => x), x => x.Length, x => x.Length, (x, y) => x + ": " + y);
            }

            {
...

                {
                    ...
                    List<Shoolar> a = new List<Shoolar>
                    {
                        new Shoolar { SchoolNumber = 11, EnterYear = 1991, Surname = "Smith" },
                        new Shoolar { SchoolNumber = 31, EnterYear = 1991, Surname = "Chernishev" },
                        new Shoolar { SchoolNumber = 61, EnterYear = 1981, Surname = "Ivanov" },
                        new Shoolar { SchoolNumber = 12, EnterYear = 1981, Surname = "Cocker" },
                        new Shoolar { SchoolNumber = 661, EnterYear = 2002, Surname = "Ellish" }
                    };

                    var years = a.Select(x => x.EnterYear).Distinct();
                    
                    IDictionary<int, int> grs = new Dictionary<int, int>();

                    foreach (var year in years)                    
                        grs.Add(year, a.Where(x => x.EnterYear == year).Count());        
